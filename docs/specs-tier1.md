...
`STCChannel` — абстракция потока сообщений (data + system), используемая для streaming, прогресса, логов и lifecycle длительных операций.

### Tier 1 (Scope)

- Поддерживается live-subscribe (без истории/replay).
- Формат сообщений: JSON.
- `canRead/canWrite` определяют доступ для конкретного участника/контекста.
- Канал может иметь конечное или бесконечное время жизни (`expiresAt?`).

История сообщений / replay — **proposal** (не Tier 1).

---

### События (Events)

Все сообщения передаются в виде envelope-событий:

- `kind: "data"` — полезная нагрузка `data`
- `kind: "system"` — системные сообщения (`hello`, `params`, `close`, и др.)

Рекомендуемые поля для отладки и агентского reasoning (не обязательны в Tier 1):
- `seq?: number` — монотонный номер сообщения на стороне отправителя
- `ts?: number` — timestamp отправителя (unix ms)

---

### System Messages (Tier 1 минимально)

- `hello` — (опционально) первичное объявление параметров канала
- `params` — уведомление об изменении параметров (`delta`)
- `close` — сигнал закрытия с кодом и причиной

`progress` — **proposal** (референс-реализация допускается в Tier 1, но потребители должны считать его опциональным).

---

### Subscription leak protection (Tier 1)

Чтобы снизить риск утечек подписок:

1) **Auto-cleanup on close (обязательно)**  
   При получении `system.close` канал обязан прекратить доставку событий и освободить ссылки на handlers подписок (даже если пользователь забыл вызвать `unsubscribe()`).

2) **Hard cap на число подписок (обязательно)**  
   Канал должен иметь ограничение на максимальное число активных подписок и предупреждать заранее:
   - warning при достижении **80%** лимита
   - hard limit по умолчанию:
     - **dev mode:** `maxSubscriptions = 100`
     - **prod mode:** `maxSubscriptions = 10000`

3) **Настройка лимита**  
   Лимит подписок должен быть настраиваемым:
   - **глобально** (дефолты для всех создаваемых каналов),
   - **локально** (override на уровне конкретного канала при создании).

TTL подписок — возможный `proposal`, но не включён в Tier 1, чтобы избежать “внезапного” отключения подписок.
